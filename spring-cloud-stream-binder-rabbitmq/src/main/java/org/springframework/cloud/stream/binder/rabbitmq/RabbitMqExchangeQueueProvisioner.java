 ...
 * @author Gary Russell
 */
public class RabbitMqExchangeQueueProvisioner {
	...
	public RabbitMqExchangeQueueProvisioner(ConnectionFactory connectionFactory) {
	...
	//@Override
	public void provisionProducerDestination(String name, ProducerProperties producerProperties) {
...
//		return new RabbitProducerDestination(exchange, binding);
	...
	//@Override
	public String provisionConsumerDestination(String name, String group, RabbitMqConsumerProperties properties) {
		boolean anonymous = !StringUtils.hasText(group);
		String  baseQueueName = anonymous ? groupedName(name, ANONYMOUS_GROUP_NAME_GENERATOR.generateName())
					: properties.getCommonProperties().isQueueNameGroupOnly() ? group : groupedName(name, group);
		if (this.logger.isInfoEnabled()) {
			this.logger.info("declaring queue for inbound: " + baseQueueName + ", bound to: " + name);
		}
		String prefix = properties.getCommonProperties().getPrefix();
		final String exchangeName = applyPrefix(prefix, name);
		Exchange exchange = buildExchange(properties.getCommonProperties(), exchangeName);
		if (properties.getCommonProperties().isDeclareExchange()) {
			declareExchange(exchangeName, exchange);
		}
		String queueName = applyPrefix(prefix, baseQueueName);
		boolean partitioned = !anonymous && properties.isPartitioned();
		boolean durable = !anonymous && properties.isDurableSubscription();
		Queue queue;
		if (anonymous) {
			queue = new Queue(queueName, false, true, true, queueArgs(queueName, properties.getCommonProperties(), false));
		}
		else {
			if (partitioned) {
			...
			if (durable) {
				queue = new Queue(queueName, true, false, false,
						queueArgs(queueName, properties.getCommonProperties(), false));
			}
			else {
		...
		Binding binding = null;
		if (properties.getCommonProperties().isBindQueue()) {
		...
		if (durable) {
			autoBindDLQ(applyPrefix(properties.getCommonProperties().getPrefix(), baseQueueName), queueName,
	...
	private Binding partitionedBinding(String destination, Exchange exchange, Queue queue,
									RabbitMqCommonProperties extendedProperties, int index) {
	...
	private Binding declareConsumerBindings(String name, RabbitMqConsumerProperties properties,
										Exchange exchange, boolean partitioned, Queue queue) {
		if (partitioned) {
			return partitionedBinding(name, exchange, queue, properties.getCommonProperties(), properties.getInstanceIndex());
		}
		else {
			return notPartitionedBinding(exchange, queue, properties.getCommonProperties());
		}
	}

	private Binding notPartitionedBinding(Exchange exchange, Queue queue, RabbitMqCommonProperties extendedProperties) {
	 ...
	 */
	private void autoBindDLQ(final String baseQueueName, String routingKey, RabbitMqCommonProperties properties) {
...
//			}
	...
	private String deadLetterExchangeName(RabbitMqCommonProperties properties) {
		if (properties.getDeadLetterExchange() == null) {
			return properties.getPrefix() + RabbitMqCommonProperties.DEAD_LETTER_EXCHANGE;
		}
		else {
	...
	private Map<String, Object> queueArgs(String queueName, RabbitMqCommonProperties properties, boolean isDlq) {
	...
	private Exchange buildExchange(RabbitMqCommonProperties properties, String exchangeName) {
		try {
